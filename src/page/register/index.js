...
function RegisterForm() {
  ...
  return <div>
      ...
      >
        ...
        className='p-loginFormCard-button m-t-10' type="primary"> 注册 </Button>
        <Link to="/user/login">
          <Button className='p-loginFormCard-button m-t-20' style={{color: '#333'}} > 已有账号，去登录 </Button>
        </Link>
      </Form>
    </div>
}
...
function BottomLink() {  const { registerType } = useContainer(RegisterStore)
  const onClickForgetPassword = () => {
    if(registerType == 1) {
      location.replace("/user/register?type=2")
    }else {
      location.replace("/user/register?type=1")
    }
  }
  return <div>
